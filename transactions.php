            ...
            case 'add':
                $stmt = $pdo->prepare("
                    INSERT INTO transaksi (
                        id_pelanggan, id_user, id_paket, tanggal_terima,
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO pengeluaran (tanggal_pengeluaran, jumlah_pengeluaran, keterangan, id_user)
                    VALUES (NOW(), ?, ?, ?)
                ");
                $stmt->execute([
                    $_POST['total_bayar'],
                    'Otomatis dari transaksi laundry',
                    $_SESSION['id_user']
                ]);
                break;
...
$query .= " ORDER BY t.tanggal_terima DESC";
...
<body>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_paket" class="form-select" required>
                                ...
                                <?php foreach ($packages as $package): ?>
                                    <option value="<?php echo $package['id_paket']; ?>"><?php echo $package['jenis_laundry']; ?> - Rp <?php echo number_format($package['harga'], 0, ',', '.'); ?>/kg</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
    ...
    <script>
        ...
        document.querySelector('select[name="id_paket"]').addEventListener('change', function() {
        ...
        document.querySelector('input[name="jumlah_kilo"]').addEventListener('input', function() {
            const package = document.querySelector('select[name="id_paket"]').options[document.querySelector('select[name="id_paket"]').selectedIndex].text;
            const price = parseFloat(package.split('Rp ')[1].replace('.', '').replace(',', '.'));
