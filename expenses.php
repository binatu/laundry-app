...
// Get all expenses without date filter
$stmt = $pdo->prepare("
...
// Get total expenses without date filter
$stmt = $pdo->prepare("SELECT COALESCE(SUM(jumlah_pengeluaran), 0) as total FROM pengeluaran");
$stmt->execute();
...
// Get total transactions without date filter
$stmt = $pdo->prepare("SELECT COALESCE(SUM(total_bayar), 0) as total FROM transaksi");
$stmt->execute();
...
<body>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <p class="text-muted">Laporan akan menampilkan data pengeluaran dan transaksi sesuai periode yang dipilih.</p>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
    ...
    <script>
        ...
        document.getElementById('printReportModal').addEventListener('show.bs.modal', function() {
            initializeDateInputs();
        });
        ...
        function printReportWithDateRange() {
            ...
            // Fetch financial report data with date filter
            const url = `get_financial_report_data.php?start_date=${startDate}&end_date=${endDate}`;
            fetch(url)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    // Use filtered expenses data
                    const filteredExpenses = data.expenses;

                    // Use filtered transactions data
                        ...
                        <body>
                            <div class="container">
                                ...
                                <div class="total-section">
                                    <h4>Ringkasan Keuangan (${dateRangeText})</h4>
                                    <div class="total-row">
